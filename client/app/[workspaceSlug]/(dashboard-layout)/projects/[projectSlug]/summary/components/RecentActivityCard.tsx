...
export default function RecentActivityCard({
    ...
    openModal({
      ...
      modalContent: (
        <div className="w-[min(920px,calc(100vw-48px))]">
          <div className="mt-5 max-h-[68vh] w-full overflow-y-auto pr-2 activity-scroll">
            {loading ? (
  ...
  return (
    <section className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-5 shadow-sm">
      ...
      <div className="mt-5 max-h-[330px] space-y-2 overflow-y-auto pr-2 activity-scroll">
        {loading ? (
          <p className="text-sm text-[var(--text-secondary)]">
            Loading activities...
          </p>
...
function groupActivitiesByDay(activities: RecentTaskActivity[]) {
  const formatter = new Intl.DateTimeFormat("en", {
    month: "short",
    day: "numeric",
    year: "numeric",
  });
  const groups = new Map<string, RecentTaskActivity[]>();

  activities.forEach((activity) => {
    const label = formatter.format(new Date(activity.createdAt));
